...
コードブロックは Blume 標準で最大高さ `24rem` に制限されます。長いサンプルを全文表示したい場合は、MDXで対象範囲を `<div className="docs-code-full">...</div>` で囲みます。
        ...
        - name: app
          ...
          env:
            ...
            - name: PLATFORM_ENV
              value: production
            - name: FEATURE_FLAGS
              value: checkout-v2,retry-budget
          volumeMounts:
            - name: config
              mountPath: /etc/payment-gateway
              readOnly: true
      volumes:
        - name: config
          configMap:
...
YAML（`docs-code-full` 指定あり、全文表示）:

<div className="docs-code-full">

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: payment-gateway-full-height
  namespace: app-prod-example
  labels:
    app.kubernetes.io/name: payment-gateway
    platform.example.internal/tier: backend
